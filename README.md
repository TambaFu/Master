...
- 現代の機械学習技術、特に深層学習はビッグデータの解析において大きな成功を収めているが、これらの手法は一般に大量のラベル付きデータを必要とし、その準備はしばしばコストがかかる。したがって、ラベルのない大規模データからパターンやクラスターを抽出することは、依然として重要な問題となっている。
...
### 応用例
- モーションセグメンテーション→独立して動いている部分を、それぞれ異なるグループとして識別。
