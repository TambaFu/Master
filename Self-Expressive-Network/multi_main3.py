...
def plot_learning_curve(data, save_path):
    ...
    color = 'tab:blue'
    ax1.set_xlabel('Epoch')  # X軸ラベルをEpochに変更
    ax1.set_ylabel('Loss', color=color)
    ax1.plot(data['epoch'], data['Loss'], color=color, label='Total Loss')
    ...
    ax2.plot(data['epoch'], data['Accuracy'], color=color, label='Clustering Accuracy')
    ax2.tick_params(axis='y', labelcolor=color)
    ax2.legend(loc='upper right')

    plt.title('Learning Curve per Epoch')  # タイトルを変更
    fig.tight_layout()
    plt.grid(True)
...
if __name__ == "__main__":  # スクリプトを直接実行したときに実行されるメイン処理
    ...
    # 追加: 再構成データ(x̂)の保存設定
    parser.add_argument('--save_xhat', action='store_true', help='各ステージの再構成データを保存する')
    parser.add_argument('--xhat_every', type=int, default=0, help='x̂保存間隔(エポック)。0の場合は最終エポックのみ保存')
    ...
    if args.dataset == 'MNIST' or args.dataset == 'FashionMNIST':
        ...
        args.__setattr__('total_iters', 60000)
    ...
    for N in [2000]:
        ...
        for epoch in pbar:  # エポックごとのループN=20000の時500回
            ...
            for stage in range(args.num_stages):
                ...
                for i in range(n_iter_per_epoch):  # ミニバッチ単位のループN=20000の時200回
                    ...
                    if n_iters % args.save_iters == 0:
                        ...
                        print("Model Saved.")
                # ステージごとの再構成結果を保存（必要に応じて）
                # xhat_every=0 の場合は「最終エポックのみ」保存
                if args.save_xhat and ((args.xhat_every > 0 and epoch % args.xhat_every == 0) or (args.xhat_every == 0 and epoch == n_epochs - 1)):
                    xhat_np = reconstructed_data.detach().cpu().numpy().astype(np.float32)
                    epoch_tag = epoch if args.xhat_every > 0 else 'final'
                    out_path = os.path.join(folder, f"xhat_{args.dataset}_N{N}_stage{stage}_epoch{epoch_tag}.npy")
                    np.save(out_path, xhat_np)
                    print(f"Saved x̂ for stage {stage} at epoch {epoch}: {out_path}")
            #""""学習曲線用の評価（エポックごとに実行）
            if N ==200:
                t = 250
            elif N ==2000:
            ...
            if epoch % t == 0:
                ...
                fulldata = torch.from_numpy(full_samples).float()
                fulldata = utils.p_normalize(fulldata)
                # 損失と精度を記録
                learning_data['epoch'].append(epoch)
                learning_data['Loss'].append(loss.item()) # エポック最後の損失を記録
                
                acc, nmi, ari = evaluate(multisenet, data=fulldata,num_stage=args.num_stages, labels=full_labels, num_subspaces=args.num_subspaces, affinity=args.affinity,
