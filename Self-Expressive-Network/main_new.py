...
class MultiSENet(nn.Module):  # Self-Expressive Networkの本体
    ...
    def forward(self, data, stage):
        ...
        for stages in range(stage):
            # 現在の入力でクエリとキーを計算
            if stages == 0:
                # 第1段階：元の入力を使用
                q = self.query_embedding(stages,queries)
                k = self.key_embedding(stages,keys)
            else:
                # 第2段階以降：前段階の再構成結果を使用
                q = self.query_embedding(stages,x_rec)
                k = self.key_embedding(stages,x_rec)  # 図では x̂_j も再構成結果
            c = self.get_coeff(q, k, stages)
            ...
            #p*k対応する要素同士掛け算
            diag_c = self.thres_list[stages]((q * k).sum(dim=1, keepdim=True)) * self.shrink
...
if __name__ == "__main__":  # スクリプトを直接実行したときに実行されるメイン処理
    ...
    epoch_n = 5000
    for N in [5000]:#200, 500, 1000, 2000, 5000, 10000, 20000]:
        #sampleをNこランダムで選ぶ
        ...
        pbar = tqdm(range(500), ncols=120)  # プログレスバー
        #c,rec_batch,reg = multisenet(queries,keys)
        for epoch in pbar:  # エポックごとのループN=20000の時500回
            ...
            c, rec_batch, reg = multisenet(batch,args.num_stages)
...
####################################################################################
            ...
            #print("Max rec_loss:", rec_loss.max().item())
            #print("Max reg:", reg.max().item())
            loss.backward()
            ...
            temp_reconstructed_data[randidx] = rec_batch.detach() # クローンした部分を更新
