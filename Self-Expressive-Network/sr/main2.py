    ...
    def forward(self, queries, keys):
        ...
        for stage in range(self.numstages):
            ...
            if stage == 0:
                ...
                # 第2段階以降：前段階の再構成結果を使用
                q = self.query_embedding(x_rec)
                k = self.key_embedding(x_rec)  # 図では x̂_j も再構成結果
...
def get_sparse_rep(multisenet, data, stage, batch_size=10, chunk_size=100, non_zeros=1000):  # 疎な自己表現係数行列を取得
    ...
    with torch.no_grad():
        ...
        for i in range(data.shape[0] // batch_size):
            ...
            q = multisenet.query_embedding(chunk)  # クエリを埋め込みに変換
            for j in range(data.shape[0] // chunk_size):
                chunk_samples = data[j * chunk_size: (j + 1) * chunk_size].cuda()
                k = multisenet.key_embedding(chunk_samples)  # キーの埋め込み
...
if __name__ == "__main__":  # スクリプトを直接実行したときに実行されるメイン処理
    ...
    for N in [200, 500, 1000, 2000, 5000, 10000, 20000]:
            ...
            for stage in range(args.num_stages):
                ...
                for i in range(n_iter_per_epoch):  # ミニバッチ単位のループN=20000の時200回
                    ...
                    if stage == 0:
                        ...
                        # 第1段階：元の入力を使用
                        q_batch = multisenet.query_embedding(batch)  # クエリ埋め込み
                        k_batch = multisenet.key_embedding(batch)  # キー埋め込み
                    else:
                        batch_idx = randidx[i * args.batch_size : (i + 1) * args.batch_size]#100このデータのインデックス
                        #インデックスに対応したデータを取得
                        batch = reconstructed_data[batch_idx].cuda()  # バッチを取得しGPUへ
                        # 第2段階以降：前段階の再構成結果を使用
                        q_batch = multisenet.query_embedding(batch)
                        k_batch = multisenet.key_embedding(batch)  # 図では x̂_j も再構成結果
                    ...
                    for j in range(n_step_per_iter):  # 再構成のために全体を分割N=20000の時2回
                        if stage == 0:
                            ...
                            k_block = multisenet.key_embedding(block)  # 各ブロックのキー
                    ...
                    reconstructed_data[batch_idx] = rec_batch.detach()
                    # 損失計算（再構成誤差＋正則化）
                    ...
                    if n_iters % args.eval_iters == 0:
                        ...
                        acc, nmi, ari = evaluate(multisenet, data=data,stage=args.num_stage, labels=labels, num_subspaces=args.num_subspaces, affinity=args.affinity,
        ...
        acc, nmi, ari = evaluate(multisenet, data=full_data,stage=args.num_stage, labels=full_labels, num_subspaces=args.num_subspaces, affinity=args.affinity,
