...
def get_sparse_rep(senet, data, batch_size=10, chunk_size=100, non_zeros=1000):  # 疎な自己表現係数行列を取得
    ...
    with torch.no_grad():
        senet.eval()  # モデルを推論モードに切り替え
        for i in range(data.shape[0] // batch_size):
            chunk = data[i * batch_size:(i + 1) * batch_size].cuda()
            q = senet.query_embedding(chunk)  # クエリを埋め込みに変換
            for j in range(data.shape[0] // chunk_size):
                chunk_samples = data[j * chunk_size: (j + 1) * chunk_size].cuda()
                k = senet.key_embedding(chunk_samples)  # キーの埋め込み
                temp = senet.get_coeff(q, k)  # 自己表現係数
...
if __name__ == "__main__":  # スクリプトを直接実行したときに実行されるメイン処理
    ...
    # 複数のサンプルサイズで実験
    for N in [20000]:#200, 500, 1000, 2000, 5000, 10000, 20000]:
        #sampleをNこランダムで選ぶ
        ...
        n_iter_per_epoch = samples.shape[0] // args.batch_size  # 1エポックあたりのイテレーション数
        n_step_per_iter = round(all_samples // block_size)  # 再構成に必要な分割数
        n_epochs = args.total_iters // n_iter_per_epoch  # 総エポック数
        ...
        for epoch in pbar:  # エポックごとのループ
            pbar.set_description(f"Epoch {epoch}")
            randidx = torch.randperm(data.shape[0])  # データをシャッフル

            for i in range(n_iter_per_epoch):  # ミニバッチ単位のループ
                senet.train()  # 訓練モードに切り替え

                batch_idx = randidx[i * args.batch_size : (i + 1) * args.batch_size]
                batch = data[batch_idx].cuda()  # バッチを取得しGPUへ
                q_batch = senet.query_embedding(batch)  # クエリ埋め込み
                k_batch = senet.key_embedding(batch)  # キー埋め込み

                rec_batch = torch.zeros_like(batch).cuda()  # 再構成バッチ初期化
                reg = torch.zeros([1]).cuda()  # 正則化項初期化

                for j in range(n_step_per_iter):  # 再構成のために全体を分割
                    block = data[j * block_size: (j + 1) * block_size].cuda()#メモリを節約しながらすべてのサンプルに対して
                    k_block = senet.key_embedding(block)  # 各ブロックのキー
                    c = senet.get_coeff(q_batch, k_block)  # 自己表現係数
                    rec_batch = rec_batch + c.mm(block)  # 再構成
                    print(block.shape)
                ...
                optimizer.zero_grad()
                loss.backward()
                nn.utils.clip_grad_norm_(senet.parameters(), 0.001)  # 勾配のノルムを制限
                optimizer.step()
